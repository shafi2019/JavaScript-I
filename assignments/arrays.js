...
let BMW_AudiAndFord = [];
for(let i = 0; i < inventory.length; i++){
    if(inventory[i].car_make === 'BMW' || inventory[i].car_make === 'Audi' || inventory[i].car_make === 'Ford'){
        BMW_AudiAndFord.push(inventory[i]);
    }
}
console.log(JSON.stringify(BMW_AudiAndFord));
//Shafi just add Ford as an extra.
